...
SC_MODULE( SysArray )
{
  ...
  //maybe add channel to connect the inter port with the PE in/out ports 
	...
	cynw_p2p <InputType>::in weight_inter[N];
	cynw_p2p <InputType>::in act_inter[N]; 
	cynw_p2p <AccumType>::out accum_inter[N];
      ...
      for (int j = 0; j < N; j++) {  // cols
        ...
        */
        if (i == 0) {  // from Array Accum (row 0, 1, N-2)
          accum_inter[j](accum_chan[i][j]);
          pe_array[i][j]->accum_in(accum_chan[i][j]);
          pe_array[i][j]->accum_out(accum_chan[i+1][j]);
        }
